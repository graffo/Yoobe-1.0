...
<form method=post name=formulario  enctype="multipart/form-data">
	...
	<div class="jumbotron">
		<h2 class="display-4">Agora os produtos!</h2>
		<p class="lead">Abaixo temos todos os produtos que você pode ter com o seu design</p>
		<hr class="my-4">
		<p><h3><span class="badge badge-secondary">2º Passo:</span> Para escolher os produtos, clique sobre eles para selecionalos</h3></p>
	<?php foreach($prods as $p){
		...
		$arrProd[]=$p->ID;		
		?>
		<input type="checkbox" class="check_produtos" id="check_<?php echo ($p->ID)?>" name="prod[]" value="<?php echo ($p->ID) .'|||'.$p->prod_mockup_type.'|||'.$attr ;?>">
		<label for="check_<?php echo ($p->ID)?>" class="produto_box">
		<div class="card" style="width: 15rem;">
		<img src="https://yoobe.co/wp-content/uploads/<?php echo $p->meta_value?>" class="card-img-top" />
			<div class="card-body">
				<h5 class="card-title"><?php echo $p->post_title ?></h5>
			</div>
		</div>
		</label>
	<?php } ?>
		...
		<button type="button" class="btn btn-success btn-lg btn-block" type="submit">Quero ver os produtos</button>
